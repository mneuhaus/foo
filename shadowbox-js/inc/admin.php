...
class ShadowboxAdmin extends Shadowbox {
	...
	function check_for_src_file () {
		...
		$option = get_option( 'shadowbox-js-missing-src' );

		if ( $status === false && ! $option )
			update_option ( 'shadowbox-js-missing-src', true );
		else if ( $status === true && $option )
	...
	function srcinfo_meta_box () {
		...
		<div id="sbgetsrcinfo">
			...
			<span class="code"><a href="http://dl.sivel.net/wordpress/plugin/shadowbox-js-src.%1$s.zip">http://dl.sivel.net/wordpress/plugin/shadowbox-js-src.%1$s.zip</a></span> and extract to <span class="code">%2$s</span>' , $this->sbversion, $srcdir ) , 'shadowbox-js' ); ?></p>
